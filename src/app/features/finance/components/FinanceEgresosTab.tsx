...
export default function FinanceEgresosTab({ stats, formData, isSubmitting, onUpdateField, onSubmit }: FinanceEgresosTabProps) {
  ...
  expenses.forEach((t) => {
    filteredByCategory[t.category] = (filteredByCategory[t.category] || 0) + Number(t.amount)
  })
      ...
      {showForm && (
        <div className="rounded-xl border border-accent/30 bg-surface p-4 sm:p-5">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
            ...
            <Input label="Monto ($)" type="number" value={formData.amount} onValueChange={(v) => onUpdateField('amount', v)} classNames={inputClasses} isRequired />
            <Input label="Fecha" type="date" value={formData.date} onValueChange={(v) => onUpdateField('date', v)} classNames={inputClasses} isRequired />
            <Select label="Categoría" selectedKeys={formData.category ? [formData.category] : []} onSelectionChange={(k) => onUpdateField('category', String(Array.from(k)[0] || ''))} classNames={selectClasses}>
              ...
              {expenses.map((t) => (
                <div key={t.id} className="flex items-center justify-between gap-3 py-3 px-1">
                  ...
                  <div className="flex items-center gap-2 shrink-0">
                    <span className="text-sm font-bold text-red-400">-${Number(t.amount).toFixed(2)}</span>
                    <Button isIconOnly size="sm" variant="light" color="danger" onPress={() => deleteMutation.mutate(t.id)}>
                      <Trash2 size={14} />
                    </Button>
                  </div>
                </div>
