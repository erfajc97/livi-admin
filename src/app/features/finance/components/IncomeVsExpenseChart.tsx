...
export default function IncomeVsExpenseChart({ totalIncome, totalExpenses }: IncomeVsExpenseChartProps) {
  ...
  if (total === 0) {
    return (
      <div className="min-w-0 rounded-xl border border-border bg-surface p-4 sm:p-5">
        <h3 className="mb-3 text-sm font-semibold text-text">Ingresos vs Egresos</h3>
        <p className="text-sm text-text-muted">Sin datos en el período seleccionado.</p>
      </div>
    )
  }
            ...
            return (
              <div key={entry.name} className="flex items-center gap-3">
                ...
                <div className="flex min-w-0 flex-1 flex-col">
                  <span className="text-xs uppercase tracking-wide text-text-muted">{entry.name}</span>
                  <div className="flex items-baseline gap-2">
                    <span className="text-base font-bold text-text">${entry.value.toFixed(2)}</span>
                    <span className="text-xs font-semibold" style={{ color: entry.color }}>{percent}%</span>
                  </div>
                </div>
              </div>
            )
