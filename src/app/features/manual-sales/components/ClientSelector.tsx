      ...
      {selectedClient ? (
        <div className="flex items-center gap-3 rounded-lg bg-bg p-3">
          ...
          <div className="flex-1">
            <p className="text-sm font-medium text-text">{selectedClient.firstName} {selectedClient.lastName}</p>
            <p className="text-xs text-text-muted">{selectedClient.email}</p>
          </div>
          <Button isIconOnly size="sm" variant="flat" onPress={() => onSelect(null)}>
            <X size={14} />
          </Button>
        </div>
      ) : (
          ...
          {(user) => (
            ...
            >
              <div className="flex flex-col">
                <span className="text-sm text-text">{user.firstName} {user.lastName}</span>
