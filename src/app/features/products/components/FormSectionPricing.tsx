...
export default function FormSectionPricing({
  formData,
  updateField,
}: FormSectionPricingProps) {
  // Solo se marca en rojo si escribieron algo inválido (vacío lo avisa el
  // resumen de arriba del botón guardar).
  const priceInvalid =
    formData.price.trim() !== '' && !(Number(formData.price) > 0)
    ...
    <div className="rounded-xl border border-border bg-surface p-5">
      ...
      <div className="flex flex-col gap-4">
        <Input
          ...
          errorMessage={
            priceInvalid ? 'El precio debe ser mayor a 0.' : undefined
          }
        />
