...
import type { LandingSection, CreateLandingSectionDto, UpdateLandingSectionDto, Product } from '../types';
...
export const landingSectionsService = {
  ...
  getActive: async (): Promise<LandingSection[]> => {
    const { data } = await axiosInstance.get<LandingSection[]>(API_ENDPOINTS.LANDING_SECTIONS_ACTIVE);
    // Si viene con wrapper, extraer el array
    return Array.isArray(data) ? data : (data as any)?.content || (data as any)?.data || [];
  },
  ...
  create: async (dto: CreateLandingSectionDto): Promise<LandingSection> => {
    const { data } = await axiosInstance.post<LandingSection>(API_ENDPOINTS.LANDING_SECTIONS, dto);
    return data;
  },

  update: async (id: number, dto: UpdateLandingSectionDto): Promise<LandingSection> => {
    const { data } = await axiosInstance.patch<LandingSection>(`${API_ENDPOINTS.LANDING_SECTION}/${id}`, dto);
    return data;
  },
  ...
  addProduct: async (sectionId: number, productId: number): Promise<LandingSection> => {
  ...
  removeProduct: async (sectionId: number, productId: number): Promise<LandingSection> => {
  ...
  getAvailableProducts: async (params?: { limit?: number; isActive?: boolean }): Promise<Product[]> => {
    try {
