...
import { useAddProductMutation, useRemoveProductMutation } from '../mutations/landingSectionsMutations';
import type { LandingSection } from '../types';

export function useSectionProductsHook(section: LandingSection | null, isModalOpen: boolean = true) {
  const [searchTerm, setSearchTerm] = useState('');
  const { data: availableProducts = [], isLoading: isLoadingProducts } = useAvailableProductsQuery(isModalOpen);
  ...
  const handleRemoveProduct = async (productId: number) => {
    if (!section) return;
    await removeProductMutation.mutateAsync({ sectionId: section.id, productId });
  };
