...
interface CreateComboPayloadWithFile extends Omit<
  CreateComboPayload,
  'imageUrl'
> {
  imageUrl?: string
...
interface UpdateComboPayloadWithFile extends Omit<
  UpdateComboPayload,
  'imageUrl'
> {
  imageUrl?: string
...
function buildFormData(
  ...
  if (payload.finalPrice !== undefined)
    formData.append('finalPrice', String(payload.finalPrice))
...
export const combosService = {
  listCombos: async (): Promise<Combo[]> => {
    const { data } = await axiosInstance.get<CoreApiResponse<Combo[]>>(
